...
class GPT(nn.Module):
    def __init__(self,
        ...
        # except:
        #     os.environ['MASTER_PORT'] = os.environ.get("PORT2",'27111')
        #     logging.info(f"address:{os.environ['MASTER_ADDR']}, port:{os.environ['MASTER_PORT']}")
        #     dist.init_process_group(backend='nccl')
        
        for i in range(7,3,-1):
            try:
                os.environ["MASTER_PORT"] = os.environ.get("PORT"+str(i),'27111')
                dist.init_process_group(backend='nccl')
                break
            except:
                logging.info(f"port:{os.environ.get('PORT'+str(i),'27111')} already been use!")

        logging.info(f"[FT][INFO] process group init success with address:{os.environ['MASTER_ADDR']}, port:{os.environ['MASTER_PORT']}")
